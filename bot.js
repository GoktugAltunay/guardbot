...
client.on("ready", async () => {
  ...
  client.user.setActivity(`V A M P İ R E `, { type: "WATCHING" });
...
client.on("voiceStateUpdate", (oldMember, newMember) => {
  ...
  if (oldUserChannel === undefined && newUserChannel !== undefined) {
    const embed = new Discord.MessageEmbed()
      .setColor("RANDOM")
      .setDescription(
        `${newMember.user} adlı kullanıcı \`${newUserChannel.name}\` isimli sesli kanala giriş yaptı!`
      );
    kanal.send(embed);
    ...
    const embed = new Discord.MessageEmbed()
      .setColor("RANDOM")
      .setDescription(
        `${newMember.user} adlı kullanıcı sesli kanaldan çıkış yaptı!`
      );
    kanal.send(embed);
  }
});
...
//Modlog Son
client.on("ready", async () => {
      let botVoiceChannel = client.channels.cache.get("827626155782832168");
